...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        #gameContainer { border:3px solid #e67e22; border-radius:12px; box-shadow:0 0 40px rgba(230,126,34,0.3); overflow:hidden; position:relative; }
        canvas { display:block; width:1000px; height:600px; background:#2c3e50; cursor:crosshair; }
        #ui { position:absolute; bottom:12px; left:0; right:0; display:flex; justify-content:space-between; padding:0 20px; pointer-events:none; color:#ecf0f1; font-size:14px; font-weight:bold; text-shadow:0 0 10px rgba(0,0,0,0.8); }
        #ui span { background:rgba(0,0,0,0.5); padding:4px 14px; border-radius:20px; backdrop-filter:blur(4px); }
        #ui .gold { color:#f1c40f; }
        #ui .hp { color:#e74c3c; }
        .controls { position:absolute; top:12px; left:50%; transform:translateX(-50%); display:flex; gap:14px; background:rgba(0,0,0,0.6); padding:8px 20px; border-radius:30px; backdrop-filter:blur(4px); pointer-events:none; }
        .controls kbd { background:#2c3e50; color:#ecf0f1; padding:4px 12px; border-radius:6px; font-size:13px; border:1px solid #555; }
        #levelDisplay { position:absolute; top:70px; right:20px; color:#f1c40f; font-size:22px; font-weight:bold; text-shadow:0 0 20px rgba(241,196,15,0.5); background:rgba(0,0,0,0.5); padding:4px 18px; border-radius:30px; backdrop-filter:blur(4px); pointer-events:none; }
    </style>
</head>
<body>
<div id="gameContainer">
    ...
    <div class="controls">
        ...
        <kbd>E</kbd> Kazma · <kbd>Q</kbd> Silah değiştir · <kbd>F</kbd> Saldır
    </div>
    <div id="ui">
...
<script>
    ...
    // --------------------------------------------------------------
    const player = {
        x: 150, y: 0, w: 30, h: 50, vx: 0, vy: 0, speed: 3.2,
        hp: 100, maxHp: 100, gold: 0, weapon: 'pickaxe',
        isFlipped: false, isWalking: false, isGrounded: true,
        animFrame: 0, swingAngle: 0, bodyLean: 0, armRaise: 0,
        attackCooldown: 0, isAttacking: false, attackTimer: 0,
        miningTimer: 0, isMining: false, stuckArrows: [],
        base: { x: 120, y: 0, hp: 300, maxHp: 300 },
        mineSlots: []
    };

    const enemy = {
        x: 850, y: 0, w: 30, h: 50, vx: 0, vy: 0, speed: 1.6,
        hp: 100, maxHp: 100, gold: 0, weapon: 'pickaxe',
        isFlipped: true, isWalking: false, isGrounded: true,
        animFrame: 0, swingAngle: 0, bodyLean: 0, armRaise: 0,
        attackCooldown: 0, isAttacking: false, attackTimer: 0,
        miningTimer: 0, isMining: false, stuckArrows: [],
        base: { x: 880, y: 0, hp: 300, maxHp: 300 },
        mineSlots: [],
        state: 'idle', targetX: 850, targetY: 0, decisionTimer: 0
    };
    ...
    // Eğer sayfa zaten yüklendiyse (onload çalışmazsa) manuel başlat
