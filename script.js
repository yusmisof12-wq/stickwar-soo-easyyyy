// multiplayer.js
// Bu dosya, ana oyun HTML'indeki <script> bloğundan SONRA yüklenmelidir,
// çünkü player, enemy, units, CMD_*, startCampaignLevel, resetLevel,
// drawStickman, canvas, ctx gibi değişken/fonksiyonları doğrudan kullanır
// (aynı sayfadaki klasik <script> etiketleri aynı üst düzey kapsamı paylaşır).
//
// MİMARİ: "Host-authoritative + snapshot relay"
// - Odadaki bir taraf (host) oyunun TEK fizik/AI simülasyonunu çalıştırır.
// - Diğer taraf (guest) kendi ekranında simülasyon ÇALIŞTIRMAZ; sadece
//   komutlarını (saldırı/savunma/geri çekil, birim satın al) sunucuya yollar
//   ve host'tan gelen "game:state" anlık görüntülerini basitçe çizer.
// - Bu yüzden guest ekranındaki animasyonlar host kadar detaylı değildir
//   (kazma/vuruş animasyonu yerine sade duruş) — bu bilinçli bir basitleştirme.
// - Co-op'ta iki oyuncunun komutları da AYNI `player` nesnesine (host'un)
//   uygulanır → altın havuzu otomatik olarak ortak olur.
// - Versus'ta guest'in komutları host'un `enemy` nesnesine uygulanır ve
//   host'taki düşman YZ'si (updateAI) devre dışı bırakılır.
...
(function () {
    ...
    const Net = {
        socket: null,
        connected: false,
        active: false,       // şu an ağ üzerinden bir oyun mu oynanıyor
        mode: null,          // 'coop' | 'versus'
        isHost: false,
        roomId: null,
        peerUsername: null,
        lastSnapshotAt: 0,
        myGold: 0,           // guest için: host'tan gelen "benim altınım" bilgisi
    };
    window.Net = Net;
    ...
    function connectSocket() {
        ...
        // ---- Arkadaşlık canlı bildirimleri ----
        Net.socket.on('friends:state', renderFriendsFromState);
        Net.socket.on('friend:request', ({ username }) => {
            refreshFriends();
            toast(`${username} sana arkadaşlık isteği gönderdi.`);
        });
        Net.socket.on('friend:accepted', ({ username }) => {
            refreshFriends();
            toast(`${username} arkadaşlık isteğini kabul etti.`);
        });
        Net.socket.on('friend:online', () => refreshFriends());
        Net.socket.on('friend:offline', () => refreshFriends());

        // ---- Sefer daveti ----
        Net.socket.on('invite:received', ({ fromUsername, level }) => {
            showInviteToast(fromUsername, level);
        });
        Net.socket.on('invite:failed', ({ toUsername }) => {
            toast(`${toUsername} şu anda çevrimdışı.`);
        });
        Net.socket.on('invite:declined', ({ byUsername }) => {
            toast(`${byUsername} daveti reddetti.`);
        ...
        // ---- Oda içi oyun senkronu ----
        Net.socket.on('room:ready', () => { /* bilgi amaçlı */ });
        Net.socket.on('room:peerLeft', () => {
            toast('Rakip/ortak bağlantıyı kesti.');
            leaveNetGame();
        });
    ...
    function toast(msg) {
        ...
        if (!el) {
            ...
            el.style.cssText = 'position:fixed;top:70px;left:50%;transform:translateX(-50%);' +
                ...
                'font-size:14px;z-index:5000;box-shadow:0 6px 16px rgba(0,0,0,0.5);';
    ...
    async function refreshFriends() {
        ...
        } catch (e) { /* giriş yapılmamış olabilir */ }
    }
