(function () {
    ...
    function leaveNetGame() {
        ...
        if (window.animationFrameId !== undefined && window.animationFrameId !== null) { 
    ...
    function wrapCommandAndBuyButtons() {
        ...
        window.setPlayerCommand = function (cmd) {
            ...
            if (typeof origSetPlayerCommand === 'function') origSetPlayerCommand(cmd);
        };

        const cmdRetreat = document.getElementById('cmdRetreat');
        if (cmdRetreat) cmdRetreat.onclick = () => window.setPlayerCommand(CMD_RETREAT);
        
        const cmdDefend = document.getElementById('cmdDefend');
        if (cmdDefend) cmdDefend.onclick = () => window.setPlayerCommand(CMD_DEFEND);
        
        const cmdAttack = document.getElementById('cmdAttack');
        if (cmdAttack) cmdAttack.onclick = () => window.setPlayerCommand(CMD_ATTACK);
