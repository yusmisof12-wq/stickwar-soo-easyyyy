   // ==================== SINIFLAR ====================
        ...
        // Orakçı (Sicklewrath) — düşük can, yüksek hasar
        class Sicklewrath {
            constructor(isPlayer, ownerIndex = 0) {
                this.isPlayer = isPlayer;
                this.ownerIndex = isPlayer ? (ownerIndex || 0) : 0;
            ...
            update() {
                // Clubman ile aynı mantık — sadece hasar/can/silah farklı
                let cmd = this.isPlayer ? unitOwnerState(this).command : enemy.command;
                let enemies = units.filter(u => u.isPlayer !== this.isPlayer && u.hp > 0 && !u.isInvulnerable);
                let enemyBase = this.isPlayer ? enemy.base : player.base;
                let myBase = this.isPlayer ? player.base : enemy.base;
                ...
                let grassTop = canvas.height - GROUND_HEIGHT;
                if (this.y < grassTop + 15) this.y = grassTop + 15;
                if (this.y > canvas.height - 20) this.y = canvas.height - 20;

                // Ambush düşmanı hep saldırır
                if (!this.isPlayer && typeof isAmbushLevel === 'function' && isAmbushLevel()) {
                    cmd = CMD_ATTACK;
                }
                ...
                if (!this.target || this.target.hp <= 0 || (this.target !== enemyBase && !visibleEnemies.includes(this.target))) {
                    this.target = null;
                    let best = Infinity;
                    for (let e of visibleEnemies) {
                        let dist = Math.hypot(e.x - this.x, e.y - this.y);
                        if (dist < best) { best = dist; this.target = e; }
                    }
                }
            ...
            draw(ctx) {
                ...
                else if (!this.isPlayer) isFlipped = true;
